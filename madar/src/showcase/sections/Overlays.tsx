...
function MenuRow({ children, danger, shortcut }: { children: React.ReactNode; danger?: boolean; shortcut?: string }) {
  return (
    <div className={danger ? 'i-danger-soft' : 'i-surface2'} style={{ display: 'flex', alignItems: 'center', gap: 10, height: 36, borderRadius: 10, padding: '0 12px', fontSize: 14, color: danger ? 'var(--danger)' : undefined, cursor: 'pointer' }}>
      {children}
  ...
  return (
    <Section label="Overlays">
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
        <button onClick={() => setModalOpen(true)} className="i-lift i-press-97" style={{ height: 44, padding: '0 22px', borderRadius: 999, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>Open modal</button>
        <button onClick={showToast} className="i-lift i-press-97" style={{ height: 44, padding: '0 22px', borderRadius: 999, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)', fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>Show toast</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 20, alignItems: 'start' }}>
        {/* command palette (static) */}
        <div style={{ borderRadius: 22, border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--shadow-4)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 11, height: 40, borderRadius: 10, padding: '0 12px', background: 'var(--accent-soft)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            ...
            <div className="i-surface2-text" style={{ display: 'flex', alignItems: 'center', gap: 11, height: 40, borderRadius: 10, padding: '0 12px', fontSize: 14, color: 'var(--text-2)', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ borderRadius: 16, border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--shadow-3)', padding: 8, maxWidth: 240 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 8, padding: 4 }}>
            <span style={{ background: 'var(--ink)', color: 'var(--on-ink)', fontSize: 12, fontWeight: 500, padding: '6px 12px', borderRadius: 8, boxShadow: 'var(--shadow-2)' }}>Tooltip — 400ms delay, rises 2px</span>
            <button style={{ height: 40, padding: '0 18px', borderRadius: 999, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Hover target</button>
          </div>
          <div style={{ borderRadius: '22px 22px 0 0', border: '1px solid var(--border)', borderBottom: 'none', background: 'var(--surface)', boxShadow: 'var(--shadow-4)', padding: '12px 20px 20px' }}>
      ...
      {modalOpen && (
        <div onClick={() => setModalOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'oklch(0.2 0.03 260 / 0.4)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'overlayIn 340ms cubic-bezier(0.22,1,0.36,1)' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 480, background: 'var(--surface)', color: 'var(--text)', borderRadius: 22, boxShadow: 'var(--shadow-4)', padding: 28, animation: 'modalIn 560ms cubic-bezier(0.22,1,0.36,1)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 6 }}>
              <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-0.01em' }}>Invite a member</div>
              <button aria-label="Close" onClick={() => setModalOpen(false)} className="i-soft-text" style={{ width: 32, height: 32, borderRadius: 999, border: 'none', background: 'var(--surface-2)', color: 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 24 }}>
              ...
              <input placeholder="name@company.com" className="i-input" style={{ height: 44, borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--surface-2)', color: 'var(--text)', padding: '0 14px', fontSize: 15 }} />
            </label>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button onClick={() => setModalOpen(false)} className="i-soft" style={{ height: 44, padding: '0 20px', borderRadius: 999, border: 'none', background: 'transparent', color: 'var(--text)', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => setModalOpen(false)} className="i-lift i-press-97" style={{ height: 44, padding: '0 22px', borderRadius: 999, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>Send invite</button>
            </div>
          </div>
        </div>
      )}
      ...
      {toastVisible && (
        <div style={{ position: 'fixed', top: 74, left: '50%', transform: 'translateX(-50%)', zIndex: 110, display: 'flex', alignItems: 'center', gap: 11, padding: '11px 18px', borderRadius: 999, background: 'var(--glass)', backdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', WebkitBackdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', border: '1px solid var(--border)', boxShadow: 'var(--shadow-3), inset 0 1px 0 var(--glass-hl)', animation: 'toastIn 340ms cubic-bezier(0.34,1.45,0.64,1)' }}>
