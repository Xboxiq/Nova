...
بدونها لا يفشل شيء ظاهريًا — وهذه هي المشكلة. الدفع إلى `main` يشغّل **ناشرَين** لا واحدًا: نشر Actions يصل حيًّا، ثم يستبدله بناء الفرع القديم بعد ثانية بجذر المستودع، فـ`index.html` يشير إلى `/src/main.tsx` والصفحة بيضاء برمز 200. لذلك يختم الـworkflow بخطوة تنتظر تجاوز تلك النافذة ثم تجلب العنوان المنشور وتفشل صراحةً إن لم يكن ما يخدمه هو البناء.
