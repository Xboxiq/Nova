...
const panel: React.CSSProperties = {
  ...
  borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', padding: 6,
...
export function Popover({ trigger, children }: { trigger: React.ReactNode; children: React.ReactNode }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <button onClick={() => setOpen((v) => !v)} aria-expanded={open} className="i-soft" style={{ height: 38, padding: '0 16px', borderRadius: 6, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>{trigger}</button>
      {open && (
  ...
  return (
    <span onMouseEnter={() => setOn(true)} onMouseLeave={() => setOn(false)} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      <span role="tooltip" style={{
        ...
        whiteSpace: 'nowrap', padding: '7px 12px', borderRadius: 6, fontSize: 12, fontWeight: 500,
        ...
        border: '1px solid var(--border)', boxShadow: 'inset 0 1px 0 var(--glass-hl)', color: 'var(--text)',
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen((v) => !v)} aria-expanded={open} style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', height: 44, borderRadius: 6,
...
export function DropdownMenu({ items }: { items: DropdownItem[] }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <button aria-label="Actions" aria-expanded={open} onClick={() => setOpen((v) => !v)} className="i-soft" style={{ width: 36, height: 36, borderRadius: 6, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text-2)', cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
      ...
      {open && (
        <div role="menu" style={panel}>
          {safe.map((it) => (
            <div key={it.label} role="menuitem" onClick={() => { it.onClick?.(); setOpen(false); }} className="i-surface2" style={{ display: 'flex', alignItems: 'center', gap: 10, height: 36, borderRadius: 6, padding: '0 12px', fontSize: 13.5, cursor: 'pointer' }}>
          ...
          {danger.map((it) => (
            <div key={it.label} role="menuitem" onClick={() => { it.onClick?.(); setOpen(false); }} className="i-danger-soft" style={{ display: 'flex', alignItems: 'center', gap: 10, height: 36, borderRadius: 6, padding: '0 12px', fontSize: 13.5, color: 'var(--danger)', cursor: 'pointer' }}>
...
export function AlertDialog({ open, title, description, confirmLabel = 'Delete', cancelLabel = 'Cancel', onConfirm, onClose }: AlertDialogProps) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'oklch(0.2 0.03 260 / 0.4)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'overlayIn 340ms cubic-bezier(0.22,1,0.36,1)' }}>
      <div onClick={(e) => e.stopPropagation()} role="alertdialog" style={{ width: '100%', maxWidth: 380, background: 'var(--surface)', color: 'var(--text)', borderRadius: 6, padding: 28, textAlign: 'center', animation: 'modalIn 560ms cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          <button onClick={onClose} className="i-soft" style={{ height: 42, padding: '0 20px', borderRadius: 6, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)', fontFamily: 'inherit', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{cancelLabel}</button>
          <button onClick={() => { onConfirm?.(); onClose(); }} className="i-lift i-press-97" style={{ height: 42, padding: '0 22px', borderRadius: 6, border: 'none', background: 'var(--danger)', color: '#fff', fontFamily: 'inherit', fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
...
export function WelcomeModal({ open, onClose, icon, title, description, ctaLabel = 'ابدأ الآن' }: { open: boolean; onClose: () => void; icon: React.ReactNode; title: React.ReactNode; description: React.ReactNode; ctaLabel?: string }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 120, background: 'oklch(0.2 0.03 260 / 0.4)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'overlayIn 340ms cubic-bezier(0.22,1,0.36,1)' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 400, background: 'var(--surface)', borderRadius: 6, overflow: 'hidden', animation: 'modalIn 560ms cubic-bezier(0.22,1,0.36,1)' }}>
        <div style={{ height: 120, display: 'grid', placeItems: 'center', background: 'linear-gradient(135deg, color-mix(in srgb, var(--accent) 30%, var(--surface)), color-mix(in srgb, var(--accent) 8%, var(--surface)))' }}>
          <span style={{ width: 58, height: 58, borderRadius: 6, background: 'var(--accent)', color: 'var(--on-accent)', display: 'grid', placeItems: 'center', }}>{icon}</span>
        </div>
        <div style={{ padding: '22px 26px 26px', textAlign: 'center' }}>
          ...
          <button onClick={onClose} className="i-lift i-press-97" style={{ width: '100%', height: 46, borderRadius: 6, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontFamily: 'inherit', fontSize: 14.5, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>{ctaLabel}</button>
        </div>
      </div>
    </div>
  );
...
export function CommandBarTrigger({ placeholder = 'Type a command or search…', kbd = '⌘K', onClick }: { placeholder?: string; kbd?: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className="i-focus-ring" style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', height: 44, borderRadius: 6, border: '1px solid var(--border-strong)', background: 'var(--surface-2)', padding: '0 8px 0 14px', color: 'var(--text-3)', fontFamily: 'inherit', fontSize: 14, cursor: 'pointer', transition: 'border-color 200ms' }}>
...
export function KbdButton({ children, kbd, onClick }: { children: React.ReactNode; kbd: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className="i-lift i-press-97" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 42, padding: '0 8px 0 18px', borderRadius: 6, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontFamily: 'inherit', fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>
      {children}
      <kbd style={{ fontSize: 11, fontWeight: 600, fontFamily: 'var(--font-mono)', borderRadius: 6, padding: '4px 8px', background: 'color-mix(in srgb, var(--on-ink) 16%, transparent)', border: '1px solid color-mix(in srgb, var(--on-ink) 22%, transparent)' }}>{kbd}</kbd>
    </button>
  );
}
...
export function SpotlightButton({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) {
  ...
  return (
    <button
      ...
      style={{
        position: 'relative', height: 46, padding: '0 26px', borderRadius: 6, border: '1px solid var(--border-strong)', overflow: 'hidden',
...
export function MessageDock({ placeholder = 'اكتب رسالتك…', onSend }: { placeholder?: string; onSend?: (text: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 6, borderRadius: 6, background: 'var(--glass)', backdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', WebkitBackdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', border: '1px solid var(--border)', boxShadow: 'inset 0 1px 0 var(--glass-hl)', }}>
      <button aria-label="Attach" className="i-soft" style={{ width: 38, height: 38, borderRadius: 6, border: 'none', background: 'transparent', color: 'var(--text-2)', cursor: 'pointer', display: 'grid', placeItems: 'center', flex: 'none' }}>
      ...
      <button
        ...
        style={{ width: 38, height: 38, borderRadius: 6, border: 'none', flex: 'none', cursor: has ? 'pointer' : 'default', display: 'grid', placeItems: 'center', background: has ? 'var(--accent)' : 'var(--surface-2)', color: has ? 'var(--on-accent)' : 'var(--text-3)', transform: has ? 'scale(1)' : 'scale(0.92)', transition: `background 250ms, color 250ms, transform 350ms ${GLIDE}` }}
      >
