...
export function SelfDrawingIcon({ paths, size = 26, pathLength = 60, durationMs = 750 }: SelfDrawingIconProps) {
  ...
  return (
    <span onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} tabIndex={0} onFocus={() => setHov(true)} onBlur={() => setHov(false)} style={{ display: 'inline-flex', padding: 12, borderRadius: 6, background: 'var(--accent-soft)', cursor: 'pointer' }}>
...
export function BlueprintCard({ children }: { children: React.ReactNode }) {
  const [hov, setHov] = useState(false);
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} tabIndex={0} onFocus={() => setHov(true)} onBlur={() => setHov(false)} style={{ position: 'relative', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', overflow: 'hidden', cursor: 'pointer' }}>
...
export function ApertureCard({ cover, children }: { cover: React.ReactNode; children: React.ReactNode }) {
  const [hov, setHov] = useState(false);
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} tabIndex={0} onFocus={() => setHov(true)} onBlur={() => setHov(false)} style={{ position: 'relative', borderRadius: 6, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', minHeight: 150 }}>
...
export function TiltedStack3D({ layers }: { layers: [React.ReactNode, React.ReactNode, React.ReactNode] }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} tabIndex={0} onFocus={() => setHov(true)} onBlur={() => setHov(false)} style={{ position: 'relative', height: 190, perspective: 800, cursor: 'pointer' }}>
...
export function FolderCard({ name, meta, chip }: { name: React.ReactNode; meta: React.ReactNode; chip?: string }) {
  const [hov, setHov] = useState(false);
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} tabIndex={0} onFocus={() => setHov(true)} onBlur={() => setHov(false)} style={{ position: 'relative', width: 190, cursor: 'pointer' }}>
