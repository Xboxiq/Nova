...
export function AiLoader({ kind = 'dots' }: { kind?: 'dots' | 'line' | 'spinner' | 'pulse' }) {
  ...
  if (kind === 'line') {
    return (
      <span role="status" aria-label="Loading" style={{ display: 'inline-block', width: 120, height: 4, borderRadius: 999, background: 'var(--surface-2)', overflow: 'hidden' }}>
        <span style={{ display: 'block', height: '100%', width: '40%', borderRadius: 999, background: 'var(--accent)', animation: 'marqueeX 1.2s cubic-bezier(0.45,0,0.55,1) infinite alternate', transformOrigin: 'left' }} />
      </span>
    );
  }
  if (kind === 'spinner') {
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', height: 10, borderRadius: 999, background: 'var(--surface-2)', overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', borderRadius: 999, background: 'linear-gradient(90deg, var(--accent), oklch(from var(--accent) calc(l + 0.1) c h), var(--accent))', backgroundSize: '200% 100%', animation: 'fluxSlide 1.8s linear infinite', transition: 'width 560ms cubic-bezier(0.22,1,0.36,1)' }} />
...
export function SyncProgressPanel({ fileName, fileMeta, progress, onCancel }: { fileName: string; fileMeta: string; progress: number; onCancel?: () => void }) {
  ...
  return (
    <div style={{ position: 'relative', borderRadius: 18, padding: '18px 18px 16px', background: 'var(--glass)', backdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', WebkitBackdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', border: '1px solid var(--border)', boxShadow: 'var(--shadow-2), inset 0 1px 0 var(--glass-hl)' }}>
      <span style={{ position: 'absolute', top: -14, insetInlineEnd: 18, width: 30, height: 30, borderRadius: 999, background: 'var(--accent)', color: 'var(--on-accent)', display: 'grid', placeItems: 'center', boxShadow: 'var(--shadow-2)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        <span style={{ width: 38, height: 38, borderRadius: 11, background: 'var(--accent-soft)', color: 'var(--accent)', display: 'grid', placeItems: 'center', flex: 'none' }}>
        ...
        <button aria-label="Cancel" onClick={onCancel} className="i-soft-text" style={{ width: 28, height: 28, borderRadius: 999, border: 'none', background: 'var(--surface-2)', color: 'var(--text-2)', cursor: 'pointer', display: 'grid', placeItems: 'center', flex: 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ flex: 1, height: 6, borderRadius: 999, background: 'var(--surface-2)' }}>
          <div style={{ width: `${pct}%`, height: '100%', borderRadius: 999, background: 'var(--accent)', transition: 'width 400ms cubic-bezier(0.22,1,0.36,1)' }} />
        </div>
...
export function RangeBar({ start, end, startLabel, endLabel }: { start: number; end: number; startLabel?: string; endLabel?: string }) {
  ...
  return (
    <div>
      <div style={{ position: 'relative', height: 6, borderRadius: 999, background: 'var(--surface-2)' }}>
        <span style={{ position: 'absolute', top: 0, bottom: 0, insetInlineStart: `${s}%`, width: `${e - s}%`, borderRadius: 999, background: 'var(--accent)' }} />
        <span style={{ position: 'absolute', top: -3, insetInlineStart: `${s}%`, width: 12, height: 12, borderRadius: '50%', background: 'var(--accent)', border: '2.5px solid var(--surface)', transform: 'translateX(-50%)', boxShadow: 'var(--shadow-1)' }} />
        <span style={{ position: 'absolute', top: -3, insetInlineStart: `${e}%`, width: 12, height: 12, borderRadius: '50%', background: 'var(--accent)', border: '2.5px solid var(--surface)', transform: 'translateX(-50%)', boxShadow: 'var(--shadow-1)' }} />
      </div>
      {(startLabel || endLabel) && (
...
export function StatusStrip({ message, onDismiss }: { message: React.ReactNode; onDismiss?: () => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', borderRadius: 999, background: 'oklch(0.2 0.03 274)', color: 'oklch(0.95 0.005 270)', fontSize: 13, fontWeight: 500 }}>
      ...
      {message}
      {onDismiss && (
        <button aria-label="Dismiss" onClick={onDismiss} style={{ marginInlineStart: 'auto', width: 22, height: 22, borderRadius: 999, border: 'none', background: 'rgba(255,255,255,0.12)', color: 'inherit', cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
      ...
      {rows.map((r, i) => (
        <div key={i} style={{ borderTop: i ? '1px solid var(--border)' : 'none' }}>
          <button onClick={() => setOpen(open === i ? -1 : i)} className="i-surface2" style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '11px 14px', border: 'none', background: 'none', fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text)', cursor: 'pointer', transition: 'background 140ms', textAlign: 'start' }}>
            ...
            <span style={{ padding: '2px 9px', borderRadius: 999, fontSize: 11, fontWeight: 700, background: `var(--${tone(r.code)}-soft)`, color: `var(--${tone(r.code)})` }}>{r.code}</span>
