...
function DockButton({ label, accent, children }: { label: string; accent?: boolean; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
      <button aria-label={label} className="i-dock-icon" style={{ width: 46, height: 46, borderRadius: 6, border: 'none', background: accent ? 'var(--accent)' : 'var(--surface)', color: accent ? 'var(--on-accent)' : 'var(--text-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 220ms cubic-bezier(0.34,1.45,0.64,1)', position: 'relative' }}>{children}</button>
  ...
  return (
    <Section label="Navigation">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 20, alignItems: 'start' }}>
        {/* mini admin shell */}
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden', display: 'grid', gridTemplateColumns: '212px 1fr', minHeight: 380 }}>
          <div style={{ background: 'var(--bg-deep)', borderInlineEnd: '1px solid var(--border)', padding: '16px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <SidebarItem>
              ...
              Customers<span style={{ marginInlineStart: 'auto', fontSize: 10.5, fontWeight: 700, background: 'var(--accent-soft)', color: 'var(--accent)', borderRadius: 6, padding: '2px 7px' }}>12</span>
            </SidebarItem>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, height: 52, padding: '0 18px', borderBottom: '1px solid var(--border)' }}>
              ...
              <span style={{ marginInlineStart: 'auto', display: 'flex', alignItems: 'center', gap: 8, height: 32, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface-2)', padding: '0 12px', color: 'var(--text-3)', fontSize: 12.5 }}>
            ...
            <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button className="i-lift" style={{ height: 32, padding: '0 14px', borderRadius: 6, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>+ New service</button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ border: '1px solid var(--border)', borderRadius: 6, padding: 14 }}>
              ...
              <div style={{ flex: 1, border: '1px solid var(--border)', borderRadius: 6, background: 'repeating-linear-gradient(45deg, var(--surface-2) 0 8px, transparent 8px 16px)', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 90 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <button aria-label="Previous" className="i-soft" style={{ width: 36, height: 36, borderRadius: 6, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <button style={{ width: 36, height: 36, borderRadius: 6, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>1</button>
              {['2', '3'].map((n) => <button key={n} className="i-soft-text" style={{ width: 36, height: 36, borderRadius: 6, border: 'none', background: 'none', color: 'var(--text-2)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{n}</button>)}
              <span style={{ color: 'var(--text-3)', fontSize: 13, padding: '0 4px' }}>…</span>
              <button className="i-soft-text" style={{ width: 36, height: 36, borderRadius: 6, border: 'none', background: 'none', color: 'var(--text-2)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>12</button>
              <button aria-label="Next" className="i-soft" style={{ width: 36, height: 36, borderRadius: 6, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ borderRadius: 6, border: '1px solid var(--border)', padding: '28px 20px', background: 'repeating-linear-gradient(45deg, var(--accent-soft) 0 10px, transparent 10px 20px), var(--bg-deep)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
            <div style={{ fontSize: 13.5, fontWeight: 500, color: 'var(--text-2)' }}>The dock — hover me</div>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, padding: 10, borderRadius: 6, background: 'var(--glass)', backdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', WebkitBackdropFilter: 'blur(var(--glass-blur)) saturate(var(--glass-sat))', border: '1px solid var(--border)', boxShadow: 'inset 0 1px 0 var(--glass-hl)', }}>
