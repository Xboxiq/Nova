...
import { FilamentSwitch } from '@/components/ui/filament-switch';
  ...
  return (
    <Section label="Imported II">
      ...
      <SpecList>
        ...
        <SpecRow name="الموجةُ الثامنة: فاتورةٌ حسابُها صحيح، وتأخيرٌ ثانيَ عشرَ لا شيءَ له، ومفتاحٌ عددُ حروفِه في الـCSS" bare specimen={
          <div className="flex w-full flex-col items-center gap-16 py-12" dir="ltr">
            <div className="flex w-full items-start justify-center rounded-2xl bg-[#eceade] px-6 pb-56 pt-24">
              <ReceiptPrinter />
            </div>
            <div className="w-full rounded-2xl bg-[#0a0a0c]">
              <FilamentSwitch />
            </div>
          </div>
        }>
          <b>حسابُ الفاتورةِ صحيحٌ إلى القِرش، وذاك يحسِمُ نوعَ هذا الملفّ.</b> خمسُ بنودٍ <code>1×5.99</code> و<code>1×21.49</code> و<code>2×6.20</code> و<code>1×3.79</code> و<code>5×1.29</code> مجموعُها <b>50.12</b>؛ وثلاثةَ عشرَ في المئةِ منها <code>6.5156</code> تُقرَّبُ إلى <b>6.52</b> المطبوعة؛ والمجموعُ <b>56.64</b>. <b>ثلاثتُها في الرفعةِ صحيحة</b> — فليست أرقامَ حَشوٍ، أحدٌ جمعَها فعلًا، فنُقِلت كما هي. و<code>td:last-child::after {'{'} content: " $" {'}'}</code> يعني <b>أن رمزَ العُملةِ ليس في الوسمِ إطلاقًا</b>: كلُّ سعرٍ رقمٌ عارٍ والعمودُ يُضيفُ رمزَه.
          <br /><br />
          <b>والتأخيرُ الميّت:</b> اثنتا عشرةَ قاعدةَ <code>.letter:nth-child(n)</code> من <code>0.05s</code> إلى <code>0.6s</code>، والكلمةُ «Printing...» <b>أحدَ عشرَ محرفًا</b>. <b>فالثانيةَ عشرةَ لا شيءَ لها لتُطبَّقَ عليه</b> — عُدَّت ولم تُفترَض، وبقيت.
          <br /><br />
          <b>والحافّتانِ الممزَّقتانِ قاعدةٌ واحدةٌ تخدُمُ الاثنتَين:</b> تدرُّجانِ خطّيّانِ عند <code>--angle</code> و<code>calc(var(--angle) * -1)</code> مُبلَّطانِ بـ<code>8px 8px</code> — <b>وهكذا يصيرُ تدرُّجانِ مستقيمانِ زِكزاكًا</b>؛ ثم يقلبُ <code>::after</code> الزاويةَ من ٤٥ إلى ٢٢٥ ويُزيحُ الموضع، <b>فترسِمُ كتلةُ التصريحاتِ نفسُها الحافّةَ السفلى</b>.
          <br /><br />
          <b>وأجودُ تفصيلةٍ في القصّ:</b> <code>clip-path: inset(100% -100px -100px -100px)</code> — مُغلَقٌ رأسيًّا تمامًا، <b>وسالبٌ في الجهاتِ الثلاثِ الأخرى</b>. <b>والقصُّ يقصُّ ظِلَّ الإسقاطِ أيضًا</b>، فالـ<code>-100px</code> موجودةٌ لتُبقيَ <code>drop-shadow</code> خارجَ القصِّ والورقةُ مخفيّةٌ. وإطارُ <code>print</code> يفتحُه إلى <code>inset(-20% …)</code>، <b>ما بعدَ الحافّة، للسببِ نفسِه</b>. وحركتانِ على العنصرِ نفسِه تتعاقبانِ بقائمةِ تأخير: <code>print 1.2s</code> عند الصِّفر و<code>display 0.4s</code> عند <code>1.35s</code> — <b>فبينهما ١٥٠ مِلّي لا تجري فيها واحدةٌ و<code>forwards</code> يُثبِّتُ الورقةَ حيث تركها الأوّل</b>. ثم يُحرِّكُ <code>display</code> الـ<code>z-index</code> إلى ٥ عند علامةِ ٧٠٪: <b>فترتفعُ الفاتورةُ أمامَ هيكلِ الطابعةِ في النهايةِ وحدَها</b>. والمُشغِّلُ <code>:has(.print-button:focus)</code> — <b><code>:focus</code> عاريةً، فتشتعِلُ بالنقرِ وبـTab كلَيهما</b>، بخلافِ زرِّ النسخِ سابقًا الذي بناه صاحبُه على <code>:focus:not(:focus-visible)</code> فعملَ للفأرةِ وحدَها. <b>فلم يحتَج شيئًا.</b>
          <br /><br />
          <b>وأكبرُ رفعةٍ في الدفعةِ لم يحتَج وسمُها تخمينًا تقريبًا.</b> <b>عددُ الحروفِ في الـCSS</b>: ثلاثُ قواعدَ تضعُ فواصلَ الكلمات — <code>state-1 span:nth-child(3)</code> و<code>state-2 span:nth-child(4)</code> و<code>(6)</code>. <b>فالفراغاتُ ليست محارفَ بل هوامشَ على حروفٍ معيَّنة</b>، ومعناه أن الأولى <b>عشرةُ</b> نطاقاتٍ تقرأُ «GetStarted» والثانيةَ <b>أحدَ عشرَ</b> تقرأُ «TimetoShine». <b>وأيُّ عددٍ آخرَ يضعُ الفواصلَ في المكانِ الخطأ.</b> <b>وعددُ البراغي في الحساب</b>: <code>calc((5 - var(--i)) * 0.2s)</code> على <code>g</code> و<code>calc((3 - var(--i)) * 0.15s)</code> على <code>.dot</code> — <b>وتعاقُبٌ معكوسٌ لا يُعقَلُ إلا بالعدِّ تنازليًّا من آخرِ فهرس</b>: فخمسُ مجموعاتٍ وثلاثُ نقاط. <b>وطولا مسارَين يُعلِنُهما شَرطا تقطيعِهما</b>: <code>100 100</code> بإزاحةٍ ١٠٠ إلى <code>100 0</code>، و<code>430 430</code> بإزاحةٍ ٤٣٠ إلى <code>860 430</code> — <b>والقيمةُ الأولى المُضاعَفةُ هي ما يُجري الشَّرطةَ دورتَين وهي تخبو</b>.
          <br /><br />
          <b>وكلُّ حرفٍ ثلاثُ نُسَخٍ من نفسِه:</b> النطاقُ <code>color: transparent</code>، و<code>::before</code> و<code>::after</code> <b>كلاهما</b> يحملُ <code>content: attr(data-label)</code>. <b>وتلك هي الميكانيكيّة:</b> الحرفُ يحتاجُ جسدَين ليَنزلِقَ الخارجُ صُعُدًا (<code>char-out</code>) والداخلُ يهبِطُ من <code>scale(10)</code> وتمويهِ عشرةٍ متجاوزًا إلى <code>-15%</code> ثم <code>+7%</code> قبلَ أن يستقرّ — <b>وعنصرٌ واحدٌ لا يكونُ في موضعَين، فأعطى صاحبُه كلَّ حرفٍ زوجًا وجعلَ العنصرَ نفسَه خفيًّا</b>. <b>والتمريرُ يدفعُ المفتاحَ نزولًا لا صعودًا:</b> السكونُ <code>translateY(-10px) scale(1.02)</code> والتمريرُ يُجلِسُه عند الصِّفر — <b>فحالةُ السكونِ هي المرفوعة</b>، وقراءتُه بالعكسِ تُشبِهُ عَيبًا فلذلك يُقال. و<code>@keyframes path-glass</code> مكتوبٌ <b>داخلَ</b> قاعدةِ <code>.part-2</code>، <b>والإطاراتُ ليست من الأشياءِ التي يُجيزُ تعشيقُ الـCSS وضعَها داخلَ قاعدةِ نمط</b> — فبقاؤها يعتمدُ على المُصرِّفِ لا على المواصفة. بقيت مُعشَّقةً كما سُلِّمت، <b>وقِيست لا افتُرِضت</b>.
        </SpecRow>
