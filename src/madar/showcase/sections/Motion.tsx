  ...
  return (
    <Section label="Motion" padding="88px 32px 96px">
      ...
      <SpecShelf>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
          <button className="i-press-94" style={{ height: 44, padding: '0 24px', borderRadius: 6, border: 'none', background: 'var(--ink)', color: 'var(--on-ink)', fontSize: 14.5, fontWeight: 600, cursor: 'pointer', transition: 'transform 140ms cubic-bezier(0.22,1,0.36,1)' }}>Press &amp; hold me</button>
          <div style={{ fontSize: 12.5, color: 'var(--text-3)', textAlign: 'center' }}>Press scale 0.97<br />140ms — feels pushable</div>
        </div>

        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
          <div style={{ width: 52, height: 52, borderRadius: 6, background: 'var(--accent)', animation: 'floaty 5s ease-in-out infinite' }} />
      ...
      <div style={{ marginTop: 56, borderRadius: 6, background: 'var(--ink)', color: 'var(--on-ink)', padding: '56px 40px', textAlign: 'center', }}>
        ...
        <button className="i-cta i-press-97" style={{ height: 48, padding: '0 26px', borderRadius: 6, border: 'none', background: 'var(--accent)', color: 'var(--on-accent)', fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'transform 220ms' }}>Open design.md</button>
      </div>
