  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ background: 'var(--bg-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: 20, cursor: 'pointer' }}>
      ...
      <div style={{ position: 'relative', paddingTop: 14 }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: 14, boxShadow: hov ? 'var(--shadow-2)' : 'var(--shadow-1)', transform: hov ? 'translateY(-7px) rotate(-1deg)' : 'none', transition: `transform 380ms ${spring}, box-shadow 380ms` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <span style={{ width: 34, height: 34, borderRadius: 6, background: 'var(--accent-soft)', color: 'var(--accent)', display: 'grid', placeItems: 'center' }}><DocIcon /></span>
...
function FannedStack() {
  ...
  const card = (base: string, fan: string, z: number): React.CSSProperties => ({
    position: 'absolute', inset: 0, borderRadius: 6, background: 'var(--surface)', border: '1px solid var(--border)', zIndex: z,
      ...
      <div style={{ position: 'relative', height: 150, margin: '10px 18px 4px' }}>
        ...
        <div style={card('rotate(0deg)', 'translateY(-10px)', 3)}>
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8, height: '100%', boxSizing: 'border-box' }}>
            <span style={{ width: 34, height: 34, borderRadius: 6, background: 'var(--accent-soft)', color: 'var(--accent)', display: 'grid', placeItems: 'center' }}><GridIcon /></span>
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ background: 'var(--bg-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: 20, cursor: 'pointer', perspective: 800 }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: 16, insetInlineStart: 16, width: 40, height: 40, borderRadius: 6, background: 'var(--accent)', color: 'var(--on-accent)', display: 'grid', placeItems: 'center', boxShadow: hov ? 'none' : 'var(--shadow-1)', transform: hov ? 'translateZ(56px)' : 'translateZ(0)', transition: `transform 420ms ${easeOut}, box-shadow 420ms` }}>
        ...
        <div style={{ position: 'absolute', insetInline: 14, bottom: 14, borderRadius: 6, padding: '10px 14px', background: 'var(--glass)', backdropFilter: 'blur(14px) saturate(150%)', WebkitBackdropFilter: 'blur(14px) saturate(150%)', border: '1px solid var(--border)', boxShadow: 'inset 0 1px 0 var(--glass-hl)', fontSize: 12.5, fontWeight: 600, transform: hov ? 'translateZ(34px)' : 'translateZ(0)', transition: `transform 420ms ${easeOut}` }}>
...
function IslandDemo() {
  return (
    <div style={{ background: 'var(--bg-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: 20 }}>
      <Label>ISLAND — pill morphs into a card (tap)</Label>
      <DynamicIsland title="طلبك في الطريق" meta="12 min">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ height: 5, borderRadius: 6, background: 'rgba(255,255,255,0.14)' }}><div style={{ width: '62%', height: 5, borderRadius: 6, background: 'oklch(0.75 0.15 158)' }} /></div>
