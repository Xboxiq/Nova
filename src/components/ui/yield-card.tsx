/* `import React` dropped: React 19 uses the automatic JSX runtime, so the
   identifier is never read, and this repo builds with `noUnusedLocals` — the file
   did not compile with it. A build fact, not a design change. Everything below
   this comment is the upload, character for character, except `aria-hidden` and
   `focusable` on the `<svg>`. */

export const YieldCard = () => {
  /* `isolate` is the one class added to the design, and it is the difference
     between the gradient border existing and not existing. The ring below is
     `-z-10` with nothing on this subtree opening a stacking context, so it
     escapes up the tree and paints beneath EVERY ancestor background it
     passes. Measured on a page whose section paints a ground: the card's 2px
     transparent border showed the page, not the ring, and with the card's own
     gradient removed the interior showed the page too — the ring was painting
     below it. It works in the author's demo because nothing above it paints.
     `isolate` on THIS wrapper — not on the card — is what puts the ring back
     where the design wants it: inside this stacking context the ring is a
     negative-z descendant, so it paints after the wrapper's (absent) ground
     and before the card, which is an in-flow block. Put `isolate` on the card
     instead and the ring paints ON TOP of the card's gradient and floods the
     whole face olive; that was measured too. */
  return (
    <div className="max-w-[400px] border-0 w-full mx-auto isolate">
      <div className="relative border-2 border-transparent rounded-[45px] p-10 bg-gradient-to-br from-[#080509] via-[#1a171c] to-[#080509]" 
           style={{
        ...
        {/* SVG Icon */}
        <div className="mb-8">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 120 120" height={80} width={80} aria-hidden="true" focusable="false">
        ...
        {/* Card Title */}
        <p className="font-semibold text-white tracking-[-0.02em] leading-10 text-[28px] pb-2">
          Get 25% Annual Return
        </p>
        
        {/* Card Description */}
        <p className="font-semibold leading-8 text-white/50 text-base max-w-[470px]">
          While your cash is deposited, delegate it to earn the highest profit
          available in the mutual funds.
        </p>
      </div>
    </div>
  );
