/* CSS only, and after the prism checkbox this is the best-behaved upload in the
   whole log: it hides its input at 1px rather than deleting it, it writes its own
   `:focus-visible`, and it writes its own `prefers-reduced-motion`. Nothing had
   to be added for a keyboard or for a screen reader.

   The tree is closed by the sibling combinator plus one physical fact:
     `.folder__toggle:checked ~ .folder__shape`   shape is the input's sibling
     `... ~ .folder__shape .folder__front`        front is inside the shape
     `... .paper--1 / --2`                        papers are inside the shape too,
                                                  under `.folder__papers`
   and `.folder__meta` is the shape's sibling, because nothing ever selects it
   through the shape.

   `.folder__toggle { pointer-events: none }` again, and it means the same thing
   it meant on the deadbolt: `.folder` is a <label>. Here that pays a second
   dividend — the meta text is INSIDE the label, so "Documents 128 items" becomes
   the control's accessible name on its own. This is the first upload in the batch
   that needed no aria-label, because its own copy does the work. */
  ...
  return (
    <StyledWrapper>
      <label className="folder">
        <input type="checkbox" className="folder__toggle" />
