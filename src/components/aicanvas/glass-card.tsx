...
/* Thirteenth upload of this batch, one of three that arrived together.

   DIVERGENCE 1. The border rotates for ever — `repeat: Infinity` over 20s — which
   is the one thing in these three that WCAG names outright. It is already handled,
   and that was measured rather than hoped: without the preference the border reads
   58, 64, 70, 76, 82 degrees across five samples; with `reduce` it reads 0 in all
   five. The showcase's `MotionConfig` reaches it because the rotation is a
   transform. Nothing to change in the upload — recorded because a perpetual
   animation is exactly what a reader would want proof about.

   DIVERGENCE 2. The subtitle failed. It is `text-white/40` over the card body, and
   composited properly it measures 3.30 against 4.5 — it is the sentence that says
   what the feature does, so it is raised to 0.65 rather than reported.

   DIVERGENCE 3. The named photograph is carried and does not arrive here
   (`naturalWidth` 0), so a local ground is drawn underneath, and the card body
   carries a dark scrim beneath its specified white film — the ground stays vivid.

   DIVERGENCE 4. The CTA had no `type`, so it defaulted to `submit`.

   DIVERGENCE 5. Five `{}` left where the registry stripped comment text out.

   Reported and not touched: the whole card is `cursor-pointer` with no handler on
   it, so it advertises a press that only its CTA actually performs. */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          /* The specified white film is kept exactly and a dark scrim goes under
             it, so the material carries the text instead of the ground being
             dimmed to do it. */
          background:
            'linear-gradient(rgba(255, 255, 255, 0.08), rgba(255, 255, 255, 0.08)), rgba(14,12,16,0.66)',
      ...
      >
        ...
        <p className="mb-5 text-sm leading-relaxed text-white/65">{subtitle}</p>
        <motion.button
          type="button"
